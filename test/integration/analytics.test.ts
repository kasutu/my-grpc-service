  ...
  const tests = [
    ...
    test('Analytics gRPC: upload batch with playback events', async () => {
      ...
      if (response.batch_id !== batchId) {
        throw new Error(`Batch ID mismatch: ${response.batch_id} !== ${batchId}`);
      }
    }),
    ...
    test('Analytics gRPC: upload batch with multiple events', async () => {
      ...
      if (response.failed_event_ids && response.failed_event_ids.length > 0) {
        throw new Error(`Some events failed: ${response.failed_event_ids.join(', ')}`);
      }
    }),
    ...
    test('Analytics gRPC: receive upload policy in response', async () => {
      ...
      if (!response.policy.max_batch_size || response.policy.max_batch_size <= 0) {
        throw new Error('Invalid max_batch_size in policy');
      }
      if (!response.policy.sync_interval_seconds || response.policy.sync_interval_seconds <= 0) {
    ...
    test('Analytics HTTP: query events with filters', async () => {
        ...
        body: JSON.stringify({
          events: [
            ...
            },
            {
              ...
              timestamp_ms: Date.now(),
              category: 'HEALTH',
              health: { battery_level: 80, cpu_usage: 10, memory_usage: 20, connection_quality: 1 },
            },
          ],
        }),
      });
      ...
      const response = await fetch(`${httpUrl}/analytics/events?device_id=${deviceId}&category=PLAYBACK`);
      if (response.status !== 200) {
        throw new Error(`Expected 200, got ${response.status}`);
      }
      ...
      const playbackEvents = body.events.filter((e: any) => e.category === 'PLAYBACK');
