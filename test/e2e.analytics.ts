...
async function runTests() {
  ...
  const tests: TestCase[] = [
        ...
        events: [
          {
            ...
            type: 'IMPRESSION',
            schema_version: 0x00010000,
            payload: encodePayload(AnalyticsFixture.createImpressionPayload({ campaignId: 'c1' })),
          },
          {
            event_id: AnalyticsFixture.generateEventIdBytes(),
            timestamp_ms: now,
            type: 'ERROR',
            schema_version: 0x00010000,
            payload: encodePayload(AnalyticsFixture.createErrorPayload({ code: 'NETWORK_ERROR' })),
          },
          {
            event_id: AnalyticsFixture.generateEventIdBytes(),
            timestamp_ms: now,
            type: 'HEARTBEAT',
            schema_version: 0x00010000,
            payload: encodePayload(AnalyticsFixture.createHeartbeatPayload({ battery: 90 })),
          },
        ],
        device_fingerprint: deviceFingerprint,
        sent_at_ms: now,
    ...
    test('Analytics v2: HTTP POST with multiple event types', async () => {
        ...
        body: JSON.stringify({
          events: [
            ...
            },
            {
              ...
              timestamp_ms: now,
              type: 'ERROR',
              payload: { code: 'DOWNLOAD_FAILED', msg: 'Network timeout', component: 'Downloader', v: 1 },
            },
            {
              event_id: AnalyticsFixture.generateUuid(),
              timestamp_ms: now,
            ...
            },
            {
              ...
              timestamp_ms: now,
              type: 'LIFECYCLE',
              payload: { action: 'app_resume', prevState: 'background', v: 1 },
            },
          ],
        }),
      });
      ...
      const body = await response.json();
      if (body.events_stored !== 5) throw new Error(`Expected 5 events stored, got ${body.events_stored}`);
    }),
    ...
    test('Analytics v2: HTTP GET /analytics/policy', async () => {
      ...
      const body = await response.json();
      if (typeof body.min_quality !== 'number') throw new Error('Missing min_quality');
      if (typeof body.max_batch_size !== 'number') throw new Error('Missing max_batch_size');
      if (typeof body.max_queue_age_hours !== 'number') throw new Error('Missing max_queue_age_hours');
      if (typeof body.upload_interval_seconds !== 'number') throw new Error('Missing upload_interval_seconds');
    }),
    ...
    test('Analytics v2: HTTP GET /analytics/events', async () => {
      ...
      const body = await response.json();
      if (!Array.isArray(body.events)) throw new Error('events should be an array');
    ...
    test('Analytics v2: HTTP GET /analytics/events with filters', async () => {
      ...
      const response = await fetch(`${httpUrl}/analytics/events?device_fingerprint=${deviceFingerprint}`);
      if (response.status !== 200) throw new Error(`Expected 200, got ${response.status}`);
      
      const body = await response.json();
      if (body.events.length !== 2) throw new Error(`Expected 2 events, got ${body.events.length}`);
      
      // Query with type filter
      const errorResponse = await fetch(`${httpUrl}/analytics/events?type=1`); // ERROR = 1
      if (errorResponse.status !== 200) throw new Error(`Expected 200, got ${errorResponse.status}`);
      
      const errorBody = await errorResponse.json();
      if (!Array.isArray(errorBody.events)) throw new Error('events should be an array');
    }),
    ...
    test('Analytics v2: HTTP GET /analytics/events/:eventId', async () => {
      ...
      await ingestAnalytics(deviceFingerprint, [{
        ...
        type: 'IMPRESSION',
        schema_version: 0x00010000,
        payload: encodePayload(AnalyticsFixture.createImpressionPayload({ campaignId: 'test-campaign' })),
      }]);
    ...
    test('Analytics v2: HTTP GET /analytics/events/:eventId returns 404 for unknown', async () => {
      const response = await fetch(`${httpUrl}/analytics/events/unknown-event-id`);
    ...
    test('Analytics v2: HTTP GET /analytics/devices/:deviceFingerprint/events', async () => {
      ...
      if (body.totalEvents !== 2) throw new Error(`Expected 2 events, got ${body.totalEvents}`);
    }),
    ...
    test('Analytics v2: HTTP GET /analytics/devices/:deviceFingerprint/summary', async () => {
      ...
      await ingestAnalytics(deviceFingerprint, [
        { event_id: AnalyticsFixture.generateEventIdBytes(), timestamp_ms: now, type: 'IMPRESSION', schema_version: 0x00010000, payload: encodePayload(AnalyticsFixture.createImpressionPayload()) },
        { event_id: AnalyticsFixture.generateEventIdBytes(), timestamp_ms: now, type: 'ERROR', schema_version: 0x00010000, payload: encodePayload(AnalyticsFixture.createErrorPayload()) },
        { event_id: AnalyticsFixture.generateEventIdBytes(), timestamp_ms: now, type: 'HEARTBEAT', schema_version: 0x00010000, payload: encodePayload(AnalyticsFixture.createHeartbeatPayload()) },
      ]);
      ...
      const body = await response.json();
      if (body.deviceFingerprint !== deviceFingerprint) throw new Error('Device fingerprint mismatch');
      if (body.totalEvents !== 3) throw new Error(`Expected 3 events, got ${body.totalEvents}`);
      ...
      if (body.eventsByType.HEARTBEAT !== 1) throw new Error('Expected 1 heartbeat');
    }),
    ...
    test('Analytics v2: HTTP GET /analytics/stats', async () => {
      ...
      const body = await response.json();
      if (typeof body.totalEvents !== 'number') throw new Error('totalEvents should be a number');
      if (typeof body.byType !== 'object') throw new Error('byType should be an object');
    }),
    ...
    test('Analytics v2: HTTP GET /analytics/fleets/:fleetId/health', async () => {
      ...
      const response = await fetch(`${httpUrl}/analytics/fleets/${fleetId}/health`);
      if (response.status !== 200) throw new Error(`Expected 200, got ${response.status}`);
      
      const body = await response.json();
      if (typeof body.totalDevices !== 'number') throw new Error('totalDevices should be a number');
      if (typeof body.onlineDevices !== 'number') throw new Error('onlineDevices should be a number');
      if (typeof body.offlineDevices !== 'number') throw new Error('offlineDevices should be a number');
      if (!Array.isArray(body.deviceHealth)) throw new Error('deviceHealth should be an array');
    }),
  ];
